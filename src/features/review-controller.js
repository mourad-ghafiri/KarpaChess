...
function classificationFor(delta) {
  if (delta < 20)   return { key: 'good',       note: 'Best or near-best move.' };
  if (delta < 60)   return { key: 'ok',         note: 'Solid move.' };
  if (delta < 150)  return { key: 'inaccuracy', note: 'Slightly inaccurate.' };
  if (delta < 300)  return { key: 'mistake',    note: 'A mistake.' };
  return              { key: 'blunder',    note: 'Blunder.' };
}
...
export class ReviewController {
  ...
  enter() {
    ...
    if (!history || history.length === 0) {
      this.bus.emit(EVENTS.TOAST, { message: 'No moves to review yet.', kind: 'info' });
      return;
    }
  ...
  async #run(history) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (isUserTurn) {
        ...
        note = c.note + (bestSan && classification !== 'good' && classification !== 'ok' ? ` Engine preferred ${bestSan}.` : '');
      } else {
      ...
      progTxt.textContent = `Analyzing move ${i + 1} of ${total}…`;
  ...
  #renderResults() {
    ...
    for (const [k, label] of labels) {
      ...
      el.innerHTML = `<span class="stat-num">${tallies[k]}</span><span class="stat-lbl">${label}</span>`;
    ...
    for (const m of this.reviewState.moves) {
      ...
      row.innerHTML = `
        ...
        <span class="rv-badge ${m.classification}">${m.classification === 'ok' ? 'Ok' : capitalize(m.classification)}</span>`;
    ...
    if (this.reviewState.selectedIdx === null) {
      ...
      if (m.bestSan && m.bestSan !== normalizeSan(m.san)) {
        html += `<br><span class="muted small">Engine's top choice: <b>${esc(m.bestSan)}</b></span>`;
      }
